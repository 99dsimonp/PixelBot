...
function DataToColor:OnInitialize()
	self:log("We're in1")
	local suc = SetMapToCurrentZone()
    self:CreateFrames(NUMBER_OF_FRAMES)
    self:log("We're in")
	--self:log(self::GetCurrentPlayerPosition())
    self:slashCommands();
end
...
function DataToColor:CreateFrames(n)
    ...
    local function UpdateFrameColor(f)
		...
		xCoordi = self:GetCurrentPlayerPosition(true)
		yCoordi = self:GetCurrentPlayerPosition(false)
...
function DataToColor:GetCurrentPlayerPosition(x)
	local do_new_map = SetMapToCurrentZone()
    --local map = C_Map.GetBestMapForUnit("player")
    local position = GetPlayerMapPosition("player")
...
function DataToColor:CorpsePosition(coord)
    ...
    if UnitIsGhost("player") then
        --local map = C_Map.GetBestMapForUnit("player")
        if GetCorpseMapPosition() ~= nil then
...
-- Returns true is player has less than 10 food in action slot 6
    ...
    if Modulo(iterator, 150) == 1 then
        if UnitIsDeadOrGhost("player") then
            ...
            if UnitIsGhost("player") then
                --local map = C_Map.GetBestMapForUnit("player")
                if GetCorpseMapPosition() ~= nil then
                    local cX, cY = GetCorpseMapPosition() -- https://wowwiki-archive.fandom.com/wiki/API_GetCorpseMapPosition
					local x = self:GetCurrentPlayerPosition(true)
					local y = self:GetCurrentPlayerPosition(false)
